...
def listar_incidentes(request):
    ...
    if request.GET.get('export') == 'pdf':
        ...
        cabeceras = ["ID", "Descripción", "Fecha", "Hora", "Ambiente", "Tipo", "Reportado por"]
        filas = [[
            str(i.id_incidente),
            (i.descripcion or "—")[:60],
...
def crear_incidente(request):
    ...
    context = {"ambientes": ambientes, "tipos": tipos, "modo_edicion": False, "hoy": _fecha_hoy()}
...
def editar_incidente(request, incidente_id):
    ...
    context = {"incidente": incidente, "ambientes": ambientes, "tipos": tipos, "modo_edicion": True, "hoy": _fecha_hoy()}
    return render(request, "formIncidente.html", context)
...
def listar_traslados(request):
    ...
    if request.GET.get('export') == 'pdf':
        ...
        cabeceras = ["ID", "Recurso", "Origen", "Destino", "Fecha", "Observación"]
        filas = [[
            str(t["id_traslado"]),
            t["recurso_nombre"],
    ...
    if request.GET.get('export') == 'excel':
        ...
        cabeceras = ["ID", "Recurso", "Serial", "Ambiente Origen", "Ambiente Destino", "Fecha", "Observación"]
        filas = [[
            str(t["id_traslado"]),
            t["recurso_nombre"],
            t["recurso_serial"],
...
def listar_ambientes(request):
    ...
    total_disponibles = sum(1 for a in ambientes_obj if (a.estado or "").lower() in ("disponible", "operativo"))
    total_ocupados = sum(1 for a in ambientes_obj if (a.estado or "").lower() in ("ocupado", "en uso"))
    ...
    if request.GET.get('export') == 'pdf':
        ...
        construir_pdf(buffer, "Ambientes de Formación", cabeceras, filas, "vertical")
        response.write(buffer.getvalue())
        return response
